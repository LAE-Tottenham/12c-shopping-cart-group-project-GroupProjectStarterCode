...
from ShopFunctions import StartShop, PrintBasket
...
class LoginState(State):
    def OnEnter(self):
        ...
        if oldBasketFilled and newBasketFilled:
            ...
            answers = questionary.select("Select an option:", choices=[prevString, currString, mergeString], style=customStyle).ask()
            ...
            if answers == prevString:
                ...
                self.context.currentAccount.Basket = mergedBasket
...
class ProductsState(State):
    def OnEnter(self):
        ...
        if self.context.currentAccount != None:
            self.context.currentAccount.Basket = self.context.currentBasket
            self.context.currentAccount.BasketCost = self.context.currentBasketCost
            DataSaving.SaveAccountsToFile(self.context.accounts)
...
class DeliveryState(State):
    def OnEnter(self):
        ...
        if len(self.context.currentAccount.OrderHistory) == 0:
            if questionary.select("Would you like to change the currency of the costs?", choices=["Confirm", "Cancel"], style=customStyle).ask() == "Confirm":
                self.context.currentAccount.Currency = questionary.text("Enter the new currency:", validate=Validation.ValidateCurrency, style=customStyle).ask().upper()
                DataSaving.SaveAccountsToFile(self.context.accounts)
        ...
        match answers:
            case "Purchase":
                ...
                self.context.currentBasketCost = 0
                DataSaving.SaveAccountsToFile(self.context.accounts)
                console.print("Purchase made successfully.", style="#98FB98 bold")
                input("Press Enter to Continue...")
                return 'menu'
            case "Change Currency":
                self.context.currentAccount.Currency = questionary.text("Enter the new currency:", validate=Validation.ValidateCurrency, style=customStyle).ask().upper()
